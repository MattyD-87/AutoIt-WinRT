...
;~ Func _WinRT_SetRestrictedErrorInfo($pIRestrictedErrorInfo)
;~ 	Local $aCall
;~ 	If @error Then Return SetError(@error, @extended, False)
;~ 	$aCall = DllCall($sDLL, "long", "SetRestrictedErrorInfo", "ptr", $pIRestrictedErrorInfo)
;~ 	If @error Then Return SetError(__WinRT_GetDllError(@error), 0, False)
;~ 	Return SetError($aCall[0], 0, $aCall[0] = $S_OK)
;~ EndFunc
