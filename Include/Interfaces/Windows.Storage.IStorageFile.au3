...
Global Const $tagIStorageFile = $tagIInspectable & _
		...
		"OpenAsync hresult(long; ptr*);" & _ ; In $iAccessMode, Out $pOperation
		...
		"CopyAsync3 hresult(ptr; handle; long; ptr*);" & _ ; In $pDestinationFolder, In $hDesiredNewName, In $iOption, Out $pOperation
		...
		"MoveAsync3 hresult(ptr; handle; long; ptr*);" & _ ; In $pDestinationFolder, In $hDesiredNewName, In $iOption, Out $pOperation
...
Func IStorageFile_OpenAsync($pThis, $iAccessMode)
	...
	Local $aCall = DllCallAddress("long", $pFunc, "ptr", $pThis, "long", $iAccessMode, "ptr*", 0)
...
Func IStorageFile_CopyAsync3($pThis, $pDestinationFolder, $sDesiredNewName, $iOption)
	...
	Local $aCall = DllCallAddress("long", $pFunc, "ptr", $pThis, "ptr", $pDestinationFolder, "handle", $hDesiredNewName, "long", $iOption, "ptr*", 0)
	Local $iError = @error
	_WinRT_DeleteHString($hDesiredNewName)
	If $iError Then Return SetError(__WinRT_GetDllError($iError), 0, $vFailVal)
